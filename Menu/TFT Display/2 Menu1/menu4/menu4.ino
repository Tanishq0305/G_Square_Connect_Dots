...
void loop() {
  ...
  if (digitalRead(BUTTON_UP) == LOW) {
    selectedItem--;
    if (selectedItem < 0) {
      selectedItem = inMuteMenu ? muteMenuItemsCount - 1 : (inSettingsMenu ? settingsMenuItemsCount - 1 : (inLevelsMenu ? levelsCount - 1 : mainMenuItemsCount - 1));
    }
    updateMenu();
  ...
  if (digitalRead(BUTTON_DOWN) == LOW) {
    selectedItem++;
    if (selectedItem >= (inMuteMenu ? muteMenuItemsCount:(inSettingsMenu ? settingsMenuItemsCount : (inLevelsMenu ? levelsCount : mainMenuItemsCount)))) {
      selectedItem = 0;
    }
    updateMenu();
    delay(200); // Debounce delay
  ...
  if (digitalRead(BUTTON_SELECT) == LOW) {
    if (!inSettingsMenu && !inLevelsMenu && selectedItem == 1) {
      ...
      displayLevelsMenu();
        ...
        selectedItem = 0;
        turnOffDisplay();
    } else if(inSettingsMenu && !inLevelsMenu && selectedItem == 0){
      inMuteMenu = true;
      selectedItem = 0;
      displayMuteMenu();
    }
